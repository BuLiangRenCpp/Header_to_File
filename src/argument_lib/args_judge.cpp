...
static bool is_htf_args(const vector<char>& args)
{
    ...
    for (char c : args) {
        ...
        if (c == args_const::INPUT_ARG)
            ...
            if (is_o)
                throw Excep_arg("args_judge.cpp::is_htf_args", "argument" + mark(c) + "only apppear once");
            is_o = true;
        }
    }
    if (is_i == false)
        throw Excep_arg("args_judge.cpp::is_htf_args", "lack of" + mark(args_const::INPUT_ARG));
    return true;
}
